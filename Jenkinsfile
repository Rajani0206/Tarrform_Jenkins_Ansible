    ...
    stages {
        stage('checkout') {
            ...
            steps {
                    script {
                        dir("terraform") {
                            // Check if the "Terrform_Jenkins_Ansible" directory exists
                            sh '''
                                if [ -d "Terrform_Jenkins_Ansible" ]; then
                                    echo "Directory exists. Deleting it..."
                                    rm -rf Terrform_Jenkins_Ansible
                                fi
                                echo "Cloning the repository..."
                                git clone "https://github.com/Rajani0206/Terrform_Jenkins_Ansible.git"
                            '''
                        }
                    }
                }
        ...
        stage('Apply') {
           ...
           steps {
                dir('terraform/Terrform_Jenkins_Ansible') {  // Add this directory block, same as in Plan stage
        ...
        steps {
         ...
         stage('Checkout Code') {
          ...
          stage('Get EC2 Public IP') {
            steps {
                script {
                    dir('terraform/Terrform_Jenkins_Ansible') {
